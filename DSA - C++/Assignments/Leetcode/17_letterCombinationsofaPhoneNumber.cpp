...
#include <string>
using namespace std;
void generate(vector<string>& final, int currDigIndex, string currString, string digits){
    ...
    for(int i = 0; i < data[digit].length(); i++){
        generate(final, currDigIndex + 1, currString + data[digit][i], digits);
    }
